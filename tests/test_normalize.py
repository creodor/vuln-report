...
class NormalizeTests(unittest.TestCase):
    def test_normalize_counts_and_limits_findings(self):
        report = json.loads(Path("fixtures/trivy-example.json").read_text(encoding="utf-8"))

        normalized = normalize_trivy_report(report, max_findings=3)

        self.assertEqual(normalized["metrics"]["total_findings"], 5)
        self.assertEqual(normalized["metrics"]["findings_sent_to_analyzer"], 3)
        self.assertEqual(normalized["metrics"]["findings_omitted_by_limit"], 2)
